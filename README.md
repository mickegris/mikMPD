...
**Songs** — Every track in the library, A–Z or Z–A by title, with an A–Z index for jumping and a filter over title, artist and album. Tap to play; swipe or long press to add to the queue, play next, or add to a playlist. Sorted by the app rather than by MPD, whose own title sort is by byte order (lowercase and accented titles end up after Z). Loaded in small pages so large libraries cannot overflow MPD's output buffer; needs MPD 0.21 or newer.
