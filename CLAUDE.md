...
- `MarqueeText` (NowPlayingView.swift) renders one-line text that auto-scrolls when it overflows; used for the Now Playing album line. State resets via `.id(text)`.
- Now Playing's square region is a three-state pane (`Pane`: art/lyrics/queue) toggled by the header buttons; art↔lyrics also toggles by tapping the pane. The tap gesture lives on the art/lyrics panes themselves, **not** the shared container — a container gesture would swallow the queue list's row taps/swipes. The queue pane reuses `QueueRow` (single tap plays; reorder stays in the Queue tab) and auto-centers the current track via `ScrollViewReader` on `playlistPos` changes.
