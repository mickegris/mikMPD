...
- **`.searchable` goes on the view's outermost container, never inside a loading branch.** A search field attached only once the list exists (after a `ProgressView`) is never installed in the navigation bar, so Artists, Genres and Playlists silently had no filter field when they opened while loading — which is every first visit. Albums and Songs attach it outside the `if loading`; the other three were moved there in v1.7.2.
