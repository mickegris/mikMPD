...
**LibraryView** — One file (LibraryView.swift, the largest view file) holding the whole Library tab: a scrolling chip bar over `LibTab` (Albums, Artists, Recent, Songs, Genres, Playlists, Radio, CD, Files — `allCases` order *is* the chip order; inserting is safe because the selected chip is plain `@State`, never persisted by index or raw value, so place a new chip where it belongs — Songs went after Recent at the owner's request), plus `AlbumListView`, `ArtistListView`, `SongListView`, `GenreListView`/`GenreDetailView`, `AlbumDetailView`, `RecentlyAddedView`, `RadioView`, `CDView`, `BrowserView`, and the shared row/grid helpers. Albums and Recently Added share a list/grid toggle persisted in `@AppStorage("libraryAlbumLayout")` (`LazyVGrid` with `GridItem(.adaptive(minimum: 130))`); sort order persists in `librarySortAlbums`/`librarySortArtists`. `RecentlyAddedView` derives `AddedAlbum`s from the bounded `loadRecentlyAdded` query and records per album whether *any* track carried an `albumartist`, since that decides the `artistTag` handed to `AlbumDetailView`.
...
- **Library chips filter through `.librarySearchable(text:prompt:)`, never a bare `.searchable`** (LibraryView.swift). Three things it and its callers must keep doing, each a bug found on device in v1.7.2:
  - **Placement `.navigationBarDrawer(displayMode: .always)`.** With the default automatic mode, visiting any chip without a filter (Recent, Radio, CD, Files) collapsed the drawer, and *no* later chip's field ever came back — not by pulling down, not by switching chips — until relaunch. Re-keying the root with `.id(tab)` did not help, and re-keying the `NavigationStack` reset the enclosing `TabView` to Now Playing.
  - **`LibraryView` ends an active search before it switches chips** (`searchPresented`, passed down as the `librarySearchPresented` environment binding), so an active search bar is never removed abruptly (UIKit: "A navigation item is losing its active search controller").
  - **Attach it outside any loading branch.** A field attached only once the list exists, after a `ProgressView`, is never installed: Artists, Genres and Playlists had no filter on first visit.
  - **No `.searchScopes` on a chip.** With the field pinned visible, the navigation bar keeps room for the hidden scope bar, and that invisible bar swallowed every tap on the chip bar below it. Songs' All/Title/Artist/Album row is a segmented `Picker` in the list instead.
...
- **Songs (Library → Songs) is the one view that holds the whole library, so it is built around three measured facts** (SongCatalog.swift). *MPD cannot send it in one response*: past `max_output_buffer_size` (8 MiB default, 0.21 included) MPD closes the connection, and the reconnect would re-ask — so `find "(file != '')" window a:b` pages of 1000 (~350 KiB), **one `Q.async` per page** so the poll interleaves. *MPD's `sort Title` is byte order*, on 0.24 as well: lowercase and Å/Ä/Ö titles sort after Z — so pages are fetched **unsorted** (the database walk; cheaper, since MPD re-sorts the whole library per sorted page) and the app sorts once, off-main, with a locale-aware comparator (`sortedCatalog`); Z–A is the reversed array. *The walk is not trusted blindly*: URIs are deduped and the total checked against `count`, with one recount (`SongCatalogWalker`, pure). **Only MPD 0.21 syntax, deliberately** — no version-switched path, so what is tested against the 0.24 server is exactly what a 0.21 server (the Chord Poly that asked for it) runs; pre-0.21 gets a message. State lives in `store.songCatalog` (its own `ObservableObject`, like `PlaybackClock`), cached per server **and `db_update`** — `connect()` runs on every foreground resume, so a per-connection cache would re-download after every unlock; one `stats` per visit catches scans from any client. A walk is invalidated by `songCatalogWalkID` (Q-only, bumped in `connect`/`disconnect`), a finish by the catalog's token (`reset()` on server switch). `SongListView` takes the store as a plain `let` and does **not** observe it — its body walks every row; a trivial `SongsTab` wrapper observes the store and passes `isConnected` down (so `LibraryView` itself still observes nothing), and the lazily built rows observe it for the playing marker. The filter has an All/Title/Artist/Album scope row (a segmented `Picker` at the top of the list — not `.searchScopes`, see Conventions; `SongFilterScope`); Artist matches `displayArtist` *or* `groupingArtist`, so compilation tracks are found by either credit. Section letters follow the comparator, not diacritic folding (`songSectionLabel`), so sections never interleave. No artwork, by decision: 10 k thumbnails would drive the art pipeline while scrolling. Cap `songCatalogLimit` = 100 000.
